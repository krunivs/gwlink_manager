...
class MultiClusterMetric(object):
    ...
    def to_object(cls, _dict):
        ...
        for key, value in _dict.items():
            if key == 'endpoints':
                for item in value:
                    endpoints.append(EndpointNetworkMetric.to_object(item))
    ...
    def get_endpoint_metric(self, name: str) -> EndpointNetworkMetric:
        """
        ...
        for i in range(0, len(self.endpoints)):
            if self.endpoints[i].get_name() == name:
                found = i

        if found < 0:
            return None

        return self.endpoints[found]
