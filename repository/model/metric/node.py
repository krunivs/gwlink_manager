...
class NodeMetric:
    ...
    def to_object(cls, _dict):
        ...
        for key, value in _dict.items():
            if key == 'cpu_metric':
                if not value:
                    setattr(instance, key, CPUMetric.get_none_object())
                else:
                    cpuMetric = CPUMetric.to_object(value)
                    items = []
                    for usage in cpuMetric.usages:
                        val = '{:.4f}'.format(usage[1])
                        items.append([usage[0], float(val)])

                    cpuMetric.usages = items
                    setattr(instance, key, cpuMetric)
            elif key == 'mem_metric':
                if not value:
                    setattr(instance, key, MemoryMetric.get_none_object())
                else:
                    memMetric = MemoryMetric.to_object(value)
                    items = []
                    for usage in memMetric.usages:
                        val = '{:.4f}'.format(usage[1])
                        items.append([usage[0], float(val)])

                    memMetric.usages = items
                    setattr(instance, key, memMetric)
            elif key == 'net_metric':
                if not value:
                    setattr(instance, key, NetworkMetric.get_none_object())
                else:
    ...
    def set_cpu_metric(self, val: CPUMetric):
        """
        setter
        :param val: (CPUMetric)
        :return:
    ...
    def get_cpu_metric(self) -> CPUMetric:
        """
        getter
        :return: (CPUMetric)
        """
    ...
    def set_memory_metric(self, val: MemoryMetric):
        """
        setter
    ...
    def get_memory_metric(self) -> MemoryMetric:
        """
        getter
        :return: (MemoryMetric)
        """
    ...
    def set_network_metric(self, val: NetworkMetric):
        """
        setter
    ...
    def get_network_metric(self) -> NetworkMetric:
        """
        getter
