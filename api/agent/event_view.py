...
class EventObject:
    ...
    def to_object(cls, _dict):
        ...
        elif _dict['object_type'] == Kubernetes.COMPONENTS.value:
            conditions = []
            for item in _dict['object_value']:
        ...
        elif _dict['object_type'] == NetStat.ENDPOINT_NETWORK.value:
            obj = EndpointNetwork.to_object(_dict['object_value'])
        ...
        elif _dict['object_type'] == NetStat.SERVICE_EXPORTS.value:
            service_exports = []
            for item in _dict['object_value']:
                service_exports.append(ServiceExport.to_object(item))
            obj = service_exports
        elif _dict['object_type'] == NetStat.SERVICE_IMPORT.value:
            obj = ServiceImport.to_object(_dict['object_value'])
        elif _dict['object_type'] == NetStat.SERVICE_IMPORTS.value: # todo
            service_imports = []
            for item in _dict['object_value']:
                service_imports.append(ServiceImport.to_object(item))
            obj = service_imports
        elif _dict['object_type'] == Metric.ENDPOINT_NETWORK_METRIC.value:
            obj = EndpointNetworkMetric.to_object(_dict['object_value'])
        else:
            raise TypeError('Invalid value({}). '
                            'Not defined in cluster.repository.model.common.type'.format(_dict['object_type']))
